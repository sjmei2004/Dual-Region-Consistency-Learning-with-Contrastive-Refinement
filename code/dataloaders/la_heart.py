...
class ElasticDeformation:
    ...
    def __call__(self, sample):
        ...
        if self.random_state.uniform() < self.execution_probability:
            ...
            return {'image': map_coordinates(image, indices, order=self.spline_order, mode='reflect'), 'label':map_coordinates(image, indices, order=0, mode='reflect')}
...
class RandomContrast:
    ...
    def __call__(self, sample):
        ...
        if self.random_state.uniform() < self.execution_probability:
            ...
            result = self.mean + alpha * (image - self.mean)
            return {'image': result, 'label': label}
