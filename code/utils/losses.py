...
def scc_loss1(cos_sim,tau,lb_center_12_bg,lb_center_12_la, un_center_12_bg, un_center_12_la):
    loss_intra_bg = torch.exp((cos_sim(lb_center_12_bg, un_center_12_bg)) / tau)
    loss_intra_la = torch.exp((cos_sim(lb_center_12_la, un_center_12_la)) / tau)
    loss_inter_bg_la = torch.exp((cos_sim(lb_center_12_bg, un_center_12_la)) / tau)
    loss_inter_la_bg = torch.exp((cos_sim(lb_center_12_la, un_center_12_bg)) / tau)
    loss_contrast_bg = -(torch.log(loss_intra_bg)) / (torch.log(loss_intra_bg) + torch.log(loss_inter_bg_la))
    loss_contrast_la = -(torch.log(loss_intra_la)) / (torch.log(loss_intra_la) + torch.log(loss_inter_la_bg))
    loss_contrast = torch.mean(loss_contrast_bg + loss_contrast_la)
    return loss_contrast

def scc_loss2(cos_sim,tau,lb_center_12_bg,lb_center_12_la, un_center_12_bg, un_center_12_la):
    loss_intra_bg = torch.exp((cos_sim(lb_center_12_bg, un_center_12_bg)) / tau)
    loss_intra_la = torch.exp((cos_sim(lb_center_12_la, un_center_12_la)) / tau)
    loss_inter_bg_la = torch.exp((cos_sim(lb_center_12_bg, un_center_12_la)) / tau)
    loss_inter_la_bg = torch.exp((cos_sim(lb_center_12_la, un_center_12_bg)) / tau)
    loss_contrast_bg = -(torch.log(loss_intra_bg / (loss_intra_bg + loss_inter_bg_la)))
    loss_contrast_la = -(torch.log(loss_intra_la / (loss_intra_la + loss_inter_la_bg)))
    loss_contrast = torch.mean(loss_contrast_bg + loss_contrast_la)
    return loss_contrast

def contrastive_loss(cos_sim,tau,anchor_bg,anchor_la, bg_hard, la_hard, bg_pos, la_pos):
    #(cos_sim, args.tau, create_center_soft_1_bg,create_center_soft_1_la, bg_hard.detach(), la_hard.detach(),bg_pos.detach(),la_pos.detach())
    loss_intra_bg = torch.exp((cos_sim(anchor_bg, bg_pos)) / tau)
    loss_intra_la = torch.exp((cos_sim(anchor_la, la_pos)) / tau)
    loss_inter_bg_la = torch.exp((cos_sim(anchor_bg, la_hard)) / tau)
    loss_inter_la_bg = torch.exp((cos_sim(anchor_la, bg_hard)) / tau)
    loss_contrast_bg = -(torch.log(loss_intra_bg / (loss_intra_bg + loss_inter_bg_la)))
    loss_contrast_la = -(torch.log(loss_intra_la / (loss_intra_la + loss_inter_la_bg)))
    loss_contrast = torch.mean(loss_contrast_bg + loss_contrast_la)
    return loss_contrast

def triplet_loss_function(lb_center_12_bg,lb_center_12_la, un_center_12_bg, un_center_12_la, margin=1.0):
    # 计算正样本和负样本之间的相似度
    pos_dist1 = torch.mm(lb_center_12_bg, un_center_12_bg.t())
    neg_dist1 = torch.mm(lb_center_12_bg, lb_center_12_la.t())
    pos_dist2 = torch.mm(lb_center_12_la, un_center_12_la.t())
    neg_dist2 = torch.mm(lb_center_12_la, lb_center_12_bg.t())
    # 正样本的损失应该小于负样本的损失，加上边界margin
    loss1 = torch.clamp(margin + pos_dist1 - neg_dist1, min=0)
    loss2 = torch.clamp(margin + pos_dist2 - neg_dist2, min=0)
    loss = loss1 + loss2
    # 对损失进行平均池化，计算平均损失
    return loss.mean()
...
def kl_divergence(alpha, num_classes, batch):
    ones = torch.ones([batch, num_classes, alpha.shape[-3], alpha.shape[-2], alpha.shape[-1]], dtype=torch.float32).cuda()
...
def edl_loss(func, y, alpha, epoch_num, num_classes, annealing_step):
    ...
    b = y.shape[0]
    kl_div = annealing_coef * kl_divergence(kl_alpha, num_classes, batch=b)#,spatial_dims=(2,3,4)
    return A + kl_div
