import argparse
import torch
from networks.E2DNet import VNet_Encoder, MainDecoder, TriupDecoder, VNet
import h5py, cv2
import math
...
parser.add_argument('--model', type=str, default='2024-11-14-14-28-12', help='model_name')
parser.add_argument('--gpu', type=str, default='0', help='GPU to use')
parser.add_argument('--normalization', type=str, default='batchnorm', help='normalization to use')
parser.add_argument('--epoch_num', type=int, default='2050', help='checkpoint to use')
...
def test_calculate_metric():
    ...
    save_mode_path = os.path.join(snapshot_path, 'SCC_best_model.pth')
    print(save_mode_path)
    pre_trained_model = torch.load(save_mode_path)
    # model.load_state_dict(pre_trained_model['model_state_dict'])
    ...
    avg_metric = dist_test_all_case(model, image_list, num_classes=num_classes,
...
def dist_test_all_case(model, image_list, num_classes, save_result=True, test_save_path=None, has_post=False):
    total_metric = 0.0
    ...
    for image_path in tqdm(image_list):
        ...
        with torch.no_grad():
            prediction, score_map = test_single_case_patch(model, image, 16, 16, 16, patch_shape,
