...
class MainDecoder(nn.Module):
    ...
    def forward(self, features):
        ...
        self.featuremap_center = x8.detach()
        return out,x8
...
class VNet2(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False):
        ...
        self.block_nine = ConvBlock(1, n_filters, n_filters, normalization=normalization)
        self.out_conv = nn.Conv3d(n_filters, n_classes, 1, padding=0)
        self.fea_conv = nn.Conv3d(32,2,1,padding=0)
        self.dropout = nn.Dropout3d(p=0.5, inplace=False)
        # self.__init_weight()
    ...
    def decoder(self, features):
        ...
        x9 = self.block_nine(x8_up)
          # 获取最后三个维度的大小
        out_size = x9.size()[-3:]
        # 使用 F.interpolate 对 x8_up 进行上采样
        # 注意：对于三维数据，插值模式可能只支持 'nearest', 'linear', 'trilinear' 等
        fea = F.interpolate(x8, size=out_size, mode='trilinear', align_corners=False)
        # x9 = F.dropout3d(x9, p=0.5, training=True)
        if self.has_dropout:
            x9 = self.dropout(x9)
        out = self.out_conv(x9)
        # fea = self.block_nine(fea)
...
class ACVNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False):
        ...
        self.encoder = VNet_Encoder(n_channels=n_channels, n_filters=n_filters, normalization=normalization, has_dropout=has_dropout)
        self.decoder1 = MainDecoder(n_classes=n_classes, n_filters=n_filters, normalization=normalization, has_dropout=has_dropout)
        self.decoder2 = TriupDecoder(n_classes=n_classes, n_filters=n_filters, normalization=normalization,has_dropout=has_dropout)
    def forward(self,x):
        features = self.encoder(x)
        output1,ft1 = self.decoder1(features)
        output2,ft2 = self.decoder2(features)
        self.featuremap_center1 = ft1
        self.featuremap_center2 = ft2
